...
class Build
{
    ...
    public static function create(App $object, $flags, $options, $tags=[]): array
    {
        ...
        $document = Build::document_use($object, $flags, $options, $document, 'package.difference_fun/parse.build.use.class');
        $document[] = '';
        $document[] = 'class '. $options->class .' {';
        $document[] = '';
        $object->config('package.difference_fun/parse.build.state.indent', 1);
        //indent++
        $document = Build::document_use($object, $flags, $options, $document, 'package.difference_fun/parse.build.use.trait');
        $document[] = '';
    ...
    {
        $object->config('package.difference_fun/parse.build.state.indent', 0);
        $document[] = '<?php';
        $document[] = '/**';
        $document[] = ' * @package Package\Difference\Fun\Parse';
        ...
        $document[] = ' * @compile-time ' . round((microtime(true) - $object->config('package.difference_fun/parse.time.start')) * 1000, 3) . ' ms';
        ...
        $document[] = ' */';
        $document[] = '';
        $document[] = 'namespace Package\Difference\Fun\Parse;';
        $document[] = '';
        return $document;
    }
        ...
        foreach($tags as $row_nr => $list){
            foreach($list as $nr => &$record){
                ...
                if(
                    ...
                    $ltrim = $object->config('package.difference_fun/parse.build.state.ltrim');
                    if($ltrim > 0){
                        $ltrim--;
                        $object->config('package.difference_fun/parse.build.state.ltrim', $ltrim);
                    }
    ...
    {
        $indent = $object->config('package.difference_fun/parse.build.state.indent');
        $document[] = str_repeat(' ', $indent * 4) . 'public function __construct(App $object, Parse $parse, Data $data, $flags, $options){';
        $object->config(
            'package.difference_fun/parse.build.state.indent',
            $object->config('package.difference_fun/parse.build.state.indent') + 1
        );
        $indent = $object->config('package.difference_fun/parse.build.state.indent');
    ...
    public static function document_run(App $object, $flags, $options, $document = [], $data = []): array
    {
        ...
        $indent++;
        $document[] = str_repeat(' ', $indent * 4) . 'throw new Exception(\'$object is not an instance of Difference\Fun\App\');';
        $indent--;
        $document[] = str_repeat(' ', $indent * 4) . '}';
        $document[] = str_repeat(' ', $indent * 4) . 'if (!($parse instanceof Parse)) {';
        $indent++;
        $document[] = str_repeat(' ', $indent * 4) . 'throw new Exception(\'$parse is not an instance of Package\Difference\Fun\Parse\Service\Parse\');';
        $indent--;
        $document[] = str_repeat(' ', $indent * 4) . '}';
        $document[] = str_repeat(' ', $indent * 4) . 'if (!($data instanceof Data)) {';
        $indent++;
        $document[] = str_repeat(' ', $indent * 4) . 'throw new Exception(\'$data is not an instance of Difference\Fun\Module\Data\');';
        $indent--;
        $document[] = str_repeat(' ', $indent * 4) . '}';
    ...
    {
        $use_class = $object->config('package.difference_fun/parse.build.use.class');
        if(empty($use_class)){
            $use_class = [];
            $use_class[] = 'Difference\Fun\App';
            $use_class[] = 'Difference\Fun\Module\Data';
            $use_class[] = 'Package\Difference\Fun\Parse\Service\Parse';
            $use_class[] = 'Plugin';
            $use_class[] = 'Exception';
        }
        $object->config('package.difference_fun/parse.build.use.class', $use_class);
        $use_trait = $object->config('package.difference_fun/parse.build.use.trait');
        if(empty($use_trait)){
            $use_trait = [];
        ...
        $object->config('package.difference_fun/parse.build.use.trait', $use_trait);
        $object->config('package.difference_fun/parse.build.state.echo', true);
        $object->config('package.difference_fun/parse.build.state.indent', 2);
    }
    ...
    {
        $is_echo = $object->config('package.difference_fun/parse.build.state.echo');
        $ltrim = $object->config('package.difference_fun/parse.build.state.ltrim');
        $skip_space = $ltrim * 4;
        if($is_echo !== true){
            return false;
...
//                return implode('echo "\n";' . PHP_EOL, $result);
    ...
    public static function plugin(App $object, $flags, $options, $record, $name): string
    {
        ...
        $use = $object->config('package.difference_fun/parse.build.use.trait');
        if(!$use){
            $use = [];
        }
        if(
        ...
        $object->config('package.difference_fun/parse.build.use.trait', $use);
